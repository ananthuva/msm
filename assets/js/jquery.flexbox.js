...
(function ($) {
    $.flexbox = function (div, o) {
        ...
        tmp_field_name = o.fieldName == '' ? $div.attr('id') : o.fieldName + "[value]";
        // The hiddenField MUST be appended to the div before the input, or IE7 does not shift the dropdown below the input field (it overlaps)
        var $hdn = $('<input type="hidden"/>')
                .attr('id', $div.attr('id') + '_hidden')
                .attr('name', tmp_field_name)
                .val(o.initialValue)
                .appendTo($div);

        tmp_field_name = o.fieldName == '' ? $div.attr('id') + "_rec" : o.fieldName + "[record]";
        var $hdn_rec = $('<input type="hidden"/>')
                .attr('id', $div.attr('id') + '_hidden_rec')
                .attr('name', tmp_field_name)
                .val(o.extraValue)
                .appendTo($div);
        delete tmp_field_name;

        o.width = (o.width > 0) ? o.width : '';
        var $input = $('<input/>')
                .attr('id', $div.attr('id') + '_input')
                .attr('autocomplete', 'off')
                .attr('placeholder', o.watermark)
                ...
                .focus(function (e) { 
                    $(".doctorformError").removeClass("formError").removeClass("validation_img");
        ...
        //HACK HR - Added Placeholder attribute
//        if (o.initialValue !== '')
...
//            $input.val(o.watermark).addClass('watermark');
        ...
        if (o.showArrow && o.showResults) {
            var arrowClick = function () {
                ...
                //Hack by HR: To clear field when clicking arrow
                //Modified On: 15 DEC 2014
                if (o.displayOptsAlways) {
                    $input.val('');
                }
                //E: hack by HR
            };
	    ...
	    //HACK BY HR: (06-04-2016) Original regex to change inorder to search hyphens and dots
        ...
        function flexbox(p, arrowOrPagingClicked, prevQuery) {
            ...
            if (q.length >= o.minChars || arrowOrPagingClicked) {
                ...
                //Hack by RCK: To clear field when clicking arrow
                //Modified On: 19 Aug 2014
                if (o.showAllOptsAlways) {
                    q = '';
                }
                //E: hack by RCK
                ...
                if (cached) {
                    ...
                    // Patch to abort the previons call - Stat
                    if (o.objDataRequest) {
                        o.objDataRequest.abort();
                    }
                    ...
                    else if (o.method.toUpperCase() == 'POST') {
                        o.objDataRequest = $.post(o.source, params, callback, "json");
                    }
                    else {
                        o.objDataRequest = $.getJSON(o.source, params, callback);
                    }
                    // Patch to abort the previons call - End

                    /*
                     * Code block before adding patch
                     * 
                     if (typeof (o.source) === 'object') {
                     if (o.allowInput)
                     callback(filter(o.source, params));
        ...
        function displayItems(d, q) {
            //HACK by HR to fix the issue in pattern while searching
            if (o.displayOptsAlways) {
                o.highlightMatchesRegExModifier = 'g';
            }
            ...
            // HACK by HR to not update hidden field value while searching
            //$hdn.val($input.val());
            ...
            for (var i = 0; i < d[o.resultsProperty].length; i++) {
                ...
                if (o.highlightMatches && o.highlightAnyMatches && !exactMatch && q !== '') {
                    var pattern,
                            highlightStart,
                            replaceString;
                    for (var j = 0; j < words.length; j++) {
                        var word = words[j];
                        highlightStart = match.toLowerCase().indexOf(word.toLowerCase()),
                                replaceString = '<span class="' + o.matchClass + '">' + match.substr(highlightStart, word.length) + '</span>'
                ...
                } else if (!exactMatch && o.highlightMatches && q !== '') {
                    var pattern = q,
    ...
    $.fn.flexbox.defaults = {
        ...
        extraValue: '',
        fieldName: '',
        showAllOptsAlways: false,
        displayOptsAlways: false,
    };

    $.fn.setValue = function (val) {
        var fbox_params = this.eq(0).prop('flexbox_conf'),
                id = '#' + this.attr('id');

        if (jQuery.isPlainObject(val) && fbox_params != undefined) {
            $(id + '_input').val(val[fbox_params.displayValue]).removeClass('watermark');
            $(id + '_hidden').val(val[fbox_params.hiddenValue]);
            $(id + '_hidden_rec').val(JSON.stringify(val));
